...
case class ProbabilityOfImprovement(eps: Double) extends AcquisitionFunction {
  ...
  def getImprovementValues(y_best: Double, mu: DenseVector[Double], s: DenseVector[Double]) = ((mu - y_best - eps)/s).map(gauss.cdf)
}

case class ExpectedImprovement(eps: Double) extends AcquisitionFunction {

  def getImprovementValues(y_best: Double, mu: DenseVector[Double], s: DenseVector[Double]) = {
